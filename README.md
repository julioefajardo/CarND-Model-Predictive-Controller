...
In order to facilitate the `cte` and `epsi` calculation, it was necessary to transform the waypoints from map coordinates to car coordinates to simplifies the process to fit a polynomial because the vehicle's origin (x, y) and orientation angle is fixed on (0,0) and 0.
